...
import { Heart, Scale, ShieldCheck, ArrowRight, Edit3, Trash2, Calculator, Package } from 'lucide-react';
        ...
        <div className="w-full md:w-64 flex flex-col justify-between p-4 rounded-xl bg-gray-50/80 dark:bg-gray-800/40 border border-gray-100 dark:border-gray-800/60 md:border-l md:border-y-0 md:border-r-0">
          ...
          <div className="flex items-center space-x-2 pt-4">
            <Link
              ...
              className="flex-1 py-2 px-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold text-xs text-center shadow-md transition-all"
            >
              View Details
            </Link>
            <button
              onClick={() => addToCompare(product)}
              className="p-2 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 hover:text-brand-600 transition-colors"
              title="Compare Product"
            >
              <Scale className="w-4 h-4" />
            </button>
            {onEdit && (
              <button onClick={() => onEdit(product)} className="p-2 rounded-xl text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-950/50">
                <Edit3 className="w-4 h-4" />
              </button>
            )}
            {onDelete && (
              <button onClick={() => onDelete(product.id)} className="p-2 rounded-xl text-red-600 hover:bg-red-50 dark:hover:bg-red-950/50">
    ...
    <div className="glass-card rounded-2xl p-4 flex flex-col justify-between group hover:border-brand-500/50 hover:shadow-xl transition-all duration-300 relative">
      ...
      <div className="pt-3 border-t border-gray-100 dark:border-gray-800/80 space-y-3">
        ...
        <div className="flex items-center space-x-2 pt-1">
          <Link
            to={`/products/${product.id}`}
            className="flex-1 py-2 px-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-semibold text-xs text-center shadow-md shadow-brand-500/20 transition-all flex items-center justify-center space-x-1"
          >
          ...
          <button
            onClick={() => addToCompare(product)}
            className="p-2 rounded-xl border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-300 hover:text-brand-600 dark:hover:text-brand-400 transition-colors"
          ...
          <button
            ...
            className="p-2 rounded-xl border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-300 hover:text-emerald-500 transition-colors"
